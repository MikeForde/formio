...
module.exports = function(formio, items, done) {
  ...
  const steps = {
    ...
    areYouSure: function(done) {
      // If running non-interactively (env provided), skip confirmation.
      ...
      inquirer.prompt([
        {
          name: 'install',
          type: 'confirm',
          message: 'Are you sure you wish to install?',
          default: false
        }
      ]).then((answers) => {
        if (!answers.install) {
    ...
    whatTemplate: function(done) {
      // Non-interactive default when env is present.
      if (process.env.ROOT_EMAIL) {
        templateFile = 'client';
        return done();
      }
